          ...
          <div className="space-y-2">
            {assignments.map((a) => (
              <div key={a.id} className="bg-gray-900 border border-white/10 rounded-lg p-3 flex items-center justify-between">
                <div>
                  <span className="text-white">{a.film?.titre || `Film #${a.id_film}`}</span>
                  <span className="text-white/40 mx-2">→</span>
                  <span className="text-purple-400">{a.jury?.username || `Jury #${a.id_jury}`}</span>
                </div>
            ...
            {votes.map((v) => (
              <div key={v.id} className="bg-gray-900 border border-white/10 rounded-lg p-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white">{v.film?.titre || `Film #${v.id_film}`}</span>
                  <span className="text-white/40">par</span>
                  <span className="text-purple-400">{v.jury?.username || `Jury #${v.id_jury}`}</span>
                </div>
